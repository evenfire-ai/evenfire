...
export const STREAM_LIMITS = {
  ...
  // Longest time a request may wait for a stream slot. A visual request can
  // wait twice (visualStreamGate, then streamGate), so the bound is chosen so
  // that two queue waits + redeem + one heartbeat interval stay below the Host
  // HTTP client's 300 s header timeout.
