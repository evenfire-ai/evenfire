 ...
 * redeem (body budget, visual gate, stream gate) is bounded by one admission
 * clock of `maxQueueWaitMs` (60 s) from arrival (#739 D1), so the queue time
 * counts once: 60 s + the 15 s redeem + one 60 s interval = 135 s, which ends
 * before the Host's 300 s headers timeout, and one interval stays below its
 * body timeout.
 */
