...
// that the ChatGPT /codex/responses endpoint rejects wider canonical MCP names.
  ...
  constructor(names: Iterable<string>) {
    ...
    for (const name of canonicalNames) {
      ...
      if (!wireNamePattern.test(name)) {
        const base = `__codex_tool_${createHash('sha256').update(name).digest('hex').slice(0, 40)}`
        wireName = base
