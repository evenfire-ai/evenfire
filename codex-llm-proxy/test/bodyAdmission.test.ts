 ...
 */
describe('codex-llm-proxy encoded bodies (R9-1)', () => {
  // A small body limit keeps these cheap; the budget scales with it.
  const maxBodyBytes = 16 * 1024

  it('T-R9-1a refuses a gzip completion body that decodes past its reservation with 415', async () => {
    const proxy = await heldProxy(maxBodyBytes)
    proxy.releaseAll()
    try {
  ...
  it('T-R9-1b refuses a gzip body on the visual parser with 415', async () => {
    const proxy = await heldProxy(maxBodyBytes)
    proxy.releaseAll()
    try {
  ...
  it('T-R9-1c refuses a gzip admin body with 415', async () => {
    const proxy = await heldProxy(maxBodyBytes)
    try {
      ...
      const refused = await postEncoded(proxy.adminPort, {
        path: '/internal/admin/v1/codex/models',
        token: adminPermit,
        body: gzipped,
        encoding: 'gzip',
      })
