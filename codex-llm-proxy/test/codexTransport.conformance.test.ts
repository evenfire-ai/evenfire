...
describe('streamCodexCompletion', () => {
  it('validates ticket bindings before redeem and maps stream frames including tool-call data', async () => {
    ...
    expect(String(fetchFn.mock.calls[0]?.[1]?.headers?.['authorization'])).toContain('tok-live')
  })
  ...
  it('does not reuse a redeemed access token across requests', async () => {
    ...
    const redeem = vi.fn(async (input: { executionTicket: string }) => {
      ...
      return redeemSuccess({ accessToken: `tok-${input.executionTicket}` })
    })
    const fetchFn = vi.fn(async () =>
    ...
    expect(String(fetchFn.mock.calls[0]?.[1]?.headers?.['authorization'])).toContain('tok-t-a')
    expect(String(fetchFn.mock.calls[1]?.[1]?.headers?.['authorization'])).toContain('tok-t-b')
  })
