...
| `HCC_AUTHORITY_MAX_STALENESS_MS`        | Maximum time a previously published MCP fleet may survive an unavailable HCC (default 180000 ms; hard-capped at 600000 ms). It must be greater than `CLERUM_CONTEXT_MAPPER_POLL_INTERVAL` in cluster mode; invalid combinations fail startup. Minikube pins 60000 ms for its deterministic staleness E2E.                                                    | `HCC_AUTHORITY_MAX_STALENESS_MS=180000`                                                                                                                                                            |
...
| `CLERUM_MODEL_NAME`                     | Overrides the default provider model name in dev mode. Used only when `CLERUM_MODEL_PROVIDER` is also set; an auto-detected provider keeps its default model.                                                                                                                                                                                                | `CLERUM_MODEL_NAME=gpt-5.4-mini`                                                                                                                                                                   |
...
| `CLERUM_MCP_TOOL_TIMEOUT_MS`            | Per-call timeout passed to MCP SDK `Client.callTool(...)` requests. The effective timeout is still bounded by any caller workflow or snippet step budget.                                                                                                                                                                                                    | `CLERUM_MCP_TOOL_TIMEOUT_MS=1500000`                                                                                                                                                               |
| `CLERUM_MCP_TOOL_MAX_TOTAL_TIMEOUT_MS`  | Maximum total MCP SDK tool-call budget used at the SDK request boundary. Operators may tune it up to the existing workflow step ceiling; invalid values fail closed at runtime.                                                                                                                                                                              | `CLERUM_MCP_TOOL_MAX_TOTAL_TIMEOUT_MS=1500000`                                                                                                                                                     |
...
| `CLERUM_CONTEXT_MAX_TOKENS`             | Token budget used by context compaction when the model's allowlist row carries no context window. `codex-subscription` and `grok-subscription` models use 256000 instead; the Host logs `context_window_resolved` with the window and its source.                                                                                                            | `CLERUM_CONTEXT_MAX_TOKENS=100000`                                                                                                                                                                 |
| `CLERUM_COMPACTION_PRE_PRUNE`           | Master switch for pre-pruning the history (deduplication, one-lining, JSON truncation, media stripping). It runs only once the history reaches the compaction threshold (80% of the context window), before any compaction tier is chosen, and at history rehydration under the same threshold. On by default; set to `false` to disable every pass at once. | `CLERUM_COMPACTION_PRE_PRUNE=true`                                                                                                                                                                 |
...
| `CLERUM_SHELL_TIMEOUT`                  | Maximum execution time in milliseconds for shell native-tool commands.                                                                                                                                                                                                                                                                                       | `CLERUM_SHELL_TIMEOUT=1500000`                                                                                                                                                                     |
| `CLERUM_TOOL_TIMEOUT`                   | Maximum execution time in milliseconds for native tool wrapper calls. This is separate from the MCP SDK request timeout controlled by `CLERUM_MCP_TOOL_TIMEOUT_MS`.                                                                                                                                                                                          | `CLERUM_TOOL_TIMEOUT=1500000`                                                                                                                                                                      |
...
| `CLERUM_ATTACHMENT_FILE_MAX_BYTES`      | Maximum decoded size in bytes of one incoming `kind:'file'` attachment; one larger file rejects the message with `FILE_ATTACHMENT_TOO_LARGE`. File base64 also counts against the 6 MiB non-image body budget, so two files at the 3 MiB default get HTTP 413 before admission. An invalid value stops the Host at startup.                                  | `CLERUM_ATTACHMENT_FILE_MAX_BYTES=3145728`                                                                                                                                                         |
| `CLERUM_FILE_REFERENCE_MAX_COUNT`       | Maximum number of `fileReferences` (structured Global Files selections, #666) one message may carry; more rejects the message with `FILE_REFERENCE_INVALID`. An invalid value stops the Host at startup.                                                                                                                                                     | `CLERUM_FILE_REFERENCE_MAX_COUNT=10`                                                                                                                                                               |
| `CLERUM_ATTACHMENT_TEXT_READ_MAX_BYTES` | Maximum bytes one `clerum__attachment_read` call returns; longer text is returned with `truncated:true` and read on with `offset`. An invalid value stops the Host at startup.                                                                                                                                                                               | `CLERUM_ATTACHMENT_TEXT_READ_MAX_BYTES=262144`                                                                                                                                                     |
...
On minikube, `make minikube-setup` writes the `chatllm` Host's `spec.model`
from `.env` through `scripts/minikube/host-model.sh`, whose per-provider
defaults are the table above. With no key it uses `openai`/`gpt-5.4-mini`; it
refuses `CLERUM_MODEL_NAME` without `CLERUM_MODEL_PROVIDER` and any pair that is
not an enabled `llm_allowed_models` row. See
[LLM providers §4.7](../docs/llm-providers/README.md#47-local-minikube-make-minikube-setup).
