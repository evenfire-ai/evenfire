...
| `CLERUM_COMPACTION_PRE_PRUNE`          | Master switch for pre-pruning the history before each LLM call (deduplication, one-lining, JSON truncation, media stripping). On by default; set to `false` to disable every pass at once.                                                                                                                | `CLERUM_COMPACTION_PRE_PRUNE=true`                                                                                                                                                                 |
