...
import { extractToolIntent, getDisplayName, sanitizeError } from '../intentExtraction.js'
...
describe('sanitizeError', () => {
  ...
  it('strips XML/HTML tags like tool_output wrappers', () => {
    const input =
      '<tool_output name="http_request" sanitized="false"> HTTP 400 {"code":400,"message":"Unknown parameter"} </tool_output>'
...
describe('extractToolIntent', () => {
  ...
  it("extracts sentence mentioning the tool's server name", () => {
    const text =
      "Let me search MongoDB for your recent orders. I'll also check Airtable for inventory."
  ...
  it('matches case-insensitively', () => {
    ...
    expect(extractToolIntent(text, 'mongodb__find')).toBe("I'll query the mongodb database now.")
  })
  ...
  it('handles native tool names by matching individual words from the tool name', () => {
    ...
    expect(extractToolIntent(text, 'file_read')).toBe("I'll read the file to check the contents.")
  })
})
