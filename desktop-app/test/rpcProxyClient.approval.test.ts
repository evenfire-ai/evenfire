...
describe("RpcProxyClient — downloadArtifact()", () => {
  it("calls GET /artifacts/:filename/download and returns Buffer", async () => {
    ...
    expect(url).toBe(
      "http://localhost:8094/api/v1/rpc/hosts/chatllm/artifacts/report.pdf/download"
    );
    expect((init.headers as Record<string, string>).authorization).toBe("Bearer rpc-token");
  ...
  it("throws on non-OK response", async () => {
    ...
    await expect(
      client.downloadArtifact("tok", "chatllm", "missing.pdf")
    ).rejects.toThrow("Download artifact failed (404): Not found");
  });
