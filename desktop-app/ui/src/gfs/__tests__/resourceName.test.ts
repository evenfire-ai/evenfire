...
import { GFS_RESOURCE_NAME_MAX_LENGTH, normalizeGfsResourceName } from '../resourceName'
...
describe('normalizeGfsResourceName', () => {
  it('keeps normal resource names unchanged', async () => {
    await expect(normalizeGfsResourceName('report.txt')).resolves.toBe('report.txt')
  })

  it('shortens oversized file names with a deterministic SHA suffix', async () => {
    const raw = `quarterly-${'very-long-'.repeat(32)}report.txt`
  ...
  it('keeps distinct oversized names distinct after truncation', async () => {
    const first = `upload-${'a'.repeat(270)}.txt`
    const second = `upload-${'a'.repeat(269)}b.txt`
