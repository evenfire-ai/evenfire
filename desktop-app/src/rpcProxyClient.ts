...
function parsePendingApproval(value: unknown, label: string): PendingApprovalLite | undefined {
  ...
  // U5 (mcp-oauth reactive consent): `reason`/`mcpServerName` are additive. They
  // ride the same REST wire as the generic approval descriptor and MUST be
  // surfaced so the renderer can branch a `connect_required` suspension into an
  // OAuth "Connect <server>" affordance instead of a generic tool approval. An
  // absent `reason` (or any value other than `connect_required`) keeps the
