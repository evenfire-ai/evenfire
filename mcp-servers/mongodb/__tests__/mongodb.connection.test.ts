...
class MockMongoClient {
   ...
   */
  async aggregate(collectionName: string, pipeline: MongoAggregatePipeline[]): Promise<any[]> {
    this.checkConnected()
...
describe('MongoDB Connection Mock', () => {
  ...
  describe('Connection Management', () => {
    ...
    it('should connect with mongodb+srv:// connection string', async () => {
      await client.connect('mongodb+srv://user:pass@cluster.mongodb.net/test')

      expect(client.connected()).toBe(true)
    })

    it('should reject invalid connection string', async () => {
      await expect(client.connect('invalid://connection-string')).rejects.toThrow(
        'Invalid connection string format'
      )
    })

    it('should reject empty connection string', async () => {
      await expect(client.connect('')).rejects.toThrow('Invalid connection string format')
    })
    ...
    it('should not allow operations when disconnected', async () => {
      await expect(client.find('test', {})).rejects.toThrow('MongoNotConnectedError')
    })
  })
...
describe('MongoDB Query Operations', () => {
  ...
  describe('Aggregate Operations', () => {
    ...
    it('should support multi-stage pipeline', async () => {
      const pipeline = [{ $match: { status: 'active' } }, { $sort: { age: 1 } }, { $limit: 2 }]
...
describe('MongoDB Write Operations', () => {
  ...
  describe('Update Operations', () => {
    ...
    it('should update a document', async () => {
      const result = await client.updateOne('users', { name: 'Grace' }, { $set: { age: 31 } })
    ...
    it('should return zero matches for non-existent document', async () => {
      const result = await client.updateOne('users', { name: 'NonExistent' }, { $set: { age: 30 } })
