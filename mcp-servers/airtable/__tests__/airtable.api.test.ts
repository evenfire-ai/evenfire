...
describe('Airtable API Mock', () => {
  ...
  describe('Mock Data Setup', () => {
    ...
    it('should set mock error for operations', async () => {
      ...
      await expect(client.listRecords('appBase123', 'tblTable1')).rejects.toThrow(
  ...
  describe('Get Record', () => {
    ...
    it('should throw error for non-existent record', async () => {
      await expect(client.getRecord('appBase123', 'tblUsers', 'recNonExistent')).rejects.toThrow(
        'NOT_FOUND'
      )
    })
  })
  ...
  describe('Update Record', () => {
    ...
    it('should update an existing record', async () => {
      const updated = await client.updateRecord('appBase123', 'tblUsers', 'rec1', { Age: 31 })
  ...
  describe('Delete Record', () => {
    ...
    it('should delete a record', async () => {
      const result = await client.deleteRecord('appBase123', 'tblUsers', 'rec1')
    ...
    it('should throw error for non-existent record', async () => {
      await expect(client.deleteRecord('appBase123', 'tblUsers', 'recNonExistent')).rejects.toThrow(
        'NOT_FOUND'
      )
    })
  })
  ...
  describe('Rate Limiting', () => {
    it('should throw rate limit error when rate limited', async () => {
      ...
      await expect(client.listRecords('appBase123', 'tblUsers')).rejects.toThrow('RATE_LIMITED')
    })
    ...
    it('should allow requests after rate limit expires', async () => {
      ...
      // Should work now
      await expect(client.listRecords('appBase123', 'tblUsers')).resolves.toBeDefined()
    })
  })
  ...
  describe('Error Handling', () => {
    it('should return authentication error', async () => {
      ...
      await expect(client.listRecords('appBase123', 'tblUsers')).rejects.toThrow(
        'AUTHENTICATION_REQUIRED'
      )
    })
    ...
    it('should return not found error', async () => {
      ...
      await expect(client.listRecords('appBase123', 'tblUsers')).rejects.toThrow('NOT_FOUND')
    })
  })
})
