...
export function createRpcRouter() {
  ...
  router.get('/rpc/hosts/:hostRef/artifacts/:filename/download', requireRpcAuth, requireScope('host:task:read'), resolveArtifactReadHost, async (_req, res) => {
  ...
  router.get('/rpc/hosts/:hostRef/artifacts/:filename/download', requireRpcAuth, requireScope('host:task:read'), wrongEndpointResolver, async (_req, res) => {
    await fetch('https://host.example/v1/runtime/artifacts/download')
    res.sendStatus(200)
  })
  ...
  router.get('/rpc/hosts/:hostRef/artifacts/:filename/download', requireRpcAuth, requireScope('host:task:read'), async (_req, res) => {
    await fetch('https://host.example/v1/runtime/artifacts/download')
    res.sendStatus(200)
  })
  return router
