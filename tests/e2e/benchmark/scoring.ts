...
export function aggregateByProvider(
  ...
  for (const [key, provResults] of grouped) {
    ...
    const avgScore = passed.length > 0 ? passed.reduce((s, r) => s + r.score, 0) / passed.length : 0
    const avgLatency =
      passed.length > 0 ? passed.reduce((s, r) => s + r.latencyMs, 0) / passed.length : 0
    const passRate = provResults.length > 0 ? passed.length / provResults.length : 0
...
export function aggregateByCategory(
  results: BenchmarkResult[]
): Map<string, Map<string, { avgScore: number; avgLatency: number; count: number }>> {
  ...
  for (const [cat, provMap] of byCat) {
    const catAgg = new Map<string, { avgScore: number; avgLatency: number; count: number }>()
    for (const [prov, rs] of provMap) {
      const passed = rs.filter(r => r.success)
      catAgg.set(prov, {
        avgScore: passed.length > 0 ? passed.reduce((s, r) => s + r.score, 0) / passed.length : 0,
        avgLatency:
          passed.length > 0 ? passed.reduce((s, r) => s + r.latencyMs, 0) / passed.length : 0,
        count: rs.length,
      })
