...
import type { BenchmarkSummary, BenchmarkResult } from "./helpers.js";
import { aggregateByProvider, aggregateByCategory } from "./scoring.js";
...
function generateMarkdown(summary: BenchmarkSummary): string {
  ...
  for (const testId of testIds) {
    ...
    for (const key of providerKeys) {
      const r = results.find(
        (res) => res.testId === testId && `${res.provider}/${res.model}` === key,
      );
  ...
  for (const [cat, provMap] of catAgg) {
    ...
    lines.push(
      `| ${formatCategory(cat)} | ${best || "N/A"} | ${bestScore.toFixed(1)} |`,
    );
  }
  lines.push("");
