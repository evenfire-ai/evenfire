...
import { CATALOG_LIMITS, listGrokModels, testGrokConnection } from '../src/grokTransport.js'
...
describe('Grok catalog upstream bounds', () => {
  it('uses a 15s deadline, a 1 MiB body cap, and the control-api model caps', () => {
  ...
  it('fails a stalled catalog request at the deadline instead of hanging', async () => {
    await expect(
      listGrokModels({ accessToken: 'tok', fetchFn: stalledFetch, lookup, timeoutMs: 50 })
    ).rejects.toMatchObject({ code: 'provider_unavailable' })
    await expect(
      testGrokConnection({ accessToken: 'tok', fetchFn: stalledFetch, lookup, timeoutMs: 50 })
    ).rejects.toMatchObject({ code: 'provider_unavailable' })
  })
  ...
  it('fails a catalog body that stalls after headers at the deadline', async () => {
    ...
    await expect(
      listGrokModels({ accessToken: 'tok', fetchFn, lookup, timeoutMs: 50 })
    ).rejects.toMatchObject({ code: 'provider_unavailable' })
  ...
  it('stops reading and fails a catalog body larger than the cap', async () => {
    ...
    await expect(listGrokModels({ accessToken: 'tok', fetchFn, lookup })).rejects.toMatchObject({
