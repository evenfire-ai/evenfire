...
function bodyWithDepth(depth: number): string {
  ...
  const body = bodyWithFiller('grok-completion-request.v1', `${'['.repeat(arrays)}${']'.repeat(arrays)}`)
...
describe('grok proxy raw-body structure bounds (A8)', () => {
  ...
  it('refuses a visual body with too many containers before JSON.parse and frees the slot', async () => {
    ...
    const over = bodyWithContainers('grok-completion-request.v2', BODY_STRUCTURE_LIMITS.maxContainers + 1)
    ...
    const atBound = bodyWithContainers('grok-completion-request.v2', BODY_STRUCTURE_LIMITS.maxContainers)
  ...
  it('refuses an ordinary body with too many containers before JSON.parse', async () => {
    ...
    const over = bodyWithContainers('grok-completion-request.v1', BODY_STRUCTURE_LIMITS.maxContainers + 1)
    ...
    const atBound = bodyWithContainers('grok-completion-request.v1', BODY_STRUCTURE_LIMITS.maxContainers)
    const admitted = await post(port, atBound)
    expect(admitted.status).toBe(403)
    expect(await admitted.json()).toEqual({ error: 'ticket_invalid' })
    expect(largeParses()).toContain(atBound.length)
  ...
  it('refuses a visual body with too many members before JSON.parse and frees the slot', async () => {
    ...
    const over = bodyWithMembers('grok-completion-request.v2', BODY_STRUCTURE_LIMITS.maxMembers + 1)
    expect(over.length).toBeGreaterThan(4096)
    const refused = await post(port, over)
    expect(refused.status).toBe(413)
    expect(await refused.json()).toEqual({ error: 'payload_too_large' })
    ...
    // Witness: the same shape at the bound is parsed and reaches the ticket
    // check through a free visual slot.
    const atBound = bodyWithMembers('grok-completion-request.v2', BODY_STRUCTURE_LIMITS.maxMembers)
    const admitted = await post(port, atBound)
    expect(admitted.status).toBe(403)
    expect(await admitted.json()).toEqual({ error: 'ticket_invalid' })
    expect(acquire).toHaveBeenCalledTimes(2)
  ...
  it('refuses an ordinary body with too many members before JSON.parse', async () => {
    ...
    const over = bodyWithMembers('grok-completion-request.v1', BODY_STRUCTURE_LIMITS.maxMembers + 1)
    expect(over.length).toBeLessThan(maxBodyBytes)
    const refused = await post(port, over)
    expect(refused.status).toBe(413)
    expect(await refused.json()).toEqual({ error: 'payload_too_large' })
    ...
    const atBound = bodyWithMembers('grok-completion-request.v1', BODY_STRUCTURE_LIMITS.maxMembers)
    const admitted = await post(port, atBound)
    expect(admitted.status).toBe(403)
    expect(await admitted.json()).toEqual({ error: 'ticket_invalid' })
    expect(largeParses()).toContain(atBound.length)
  ...
  it('refuses a charset other than UTF-8 with 415', async () => {
    const port = listen(createProxyApps(config()))
    const body = bodyWithFiller('grok-completion-request.v1', '[]')
  ...
  it('never logs the raw body of a refused request', async () => {
    ...
    const port = listen(createProxyApps(config({ maxBodyBytes: 4096 })))
    const over = bodyWithContainers('grok-completion-request.v2', BODY_STRUCTURE_LIMITS.maxContainers + 1, MARKER)
    const refused = await post(port, over)
    expect(refused.status).toBe(413)
    expect(warn).toHaveBeenCalledWith(
      { event: 'grok_proxy_denied', code: 'payload_too_large' },
      'request denied'
    )
  ...
  it('logs only the type and status of an unmapped verify error', async () => {
    ...
    const res = await post(port, bodyWithFiller('grok-completion-request.v1', '[]', { content: MARKER }))
    expect(res.status).toBe(500)
    expect(await res.json()).toEqual({ error: 'internal_error' })
    expect(error).toHaveBeenCalledWith(
      { event: 'grok_proxy_error', type: 'entity.verify.failed', status: 403 },
  ...
  describe('a demoted visual body (declared above the ordinary cap, compact below it)', () => {
    ...
    async function fillBodyBudget(port: number, maxBodyBytes: number, stream: ReturnType<typeof controlledStream>) {
      const bodies = [0, 1, 2].map(() =>
        bodyWithFiller('grok-completion-request.v1', '[]', { executionTicket: ticket() })
      )
    ...
    function ordinaryCapFor(): number {
      ...
      return Buffer.byteLength(bodyWithFiller('grok-completion-request.v1', '[]', { executionTicket: ticket() })) + 8
    }

    function demotedV2(maxBodyBytes: number, expiresIn = 60): string {
      const compact = bodyWithFiller('grok-completion-request.v2', '[]', { executionTicket: ticket(expiresIn) })
