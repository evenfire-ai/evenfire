...
function canonicalIssueResponse(access = 'a.b.c', refresh = 'd.e.f', control = 'g.h.i') {
  return {
...
describe('mcpHostRuntimeTokenIssuerClient (HCC)', () => {
  ...
  it('throws when INTERNAL_CONTROL_JWT_HCC_HMAC_SECRET is empty', async () => {
    ...
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(
  ...
  it('throws when INTERNAL_CONTROL_JWT_HCC_HMAC_SECRET still has the canary placeholder', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(/placeholder value/)
  })
  ...
  it('posts to the default sentinel issuance path with bearer InternalControl JWT', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    const result = await issueMcpHostRuntimeTokens('chatllm', [
      'workflow:list',
      'workflow:read',
      'workflow:trigger',
    ])

    expect(result.accessToken).toBe('a.b.c')
    ...
    expect((init as RequestInit).body).toBe(
      '{"includeMcpHostControlToken":true,"host":"chatllm","workflowControlScopes":["workflow:list","workflow:read","workflow:trigger"]}'
    )
  ...
  it('forwards derived llm:codex:execute in the issuance body', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await issueMcpHostRuntimeTokens('chatllm', ['workflow:list', 'llm:codex:execute'])

    const [, init] = fetchMock.mock.calls[0]
    expect((init as RequestInit).body).toBe(
      '{"includeMcpHostControlToken":true,"host":"chatllm","workflowControlScopes":["workflow:list","llm:codex:execute"]}'
    )
  })

  it('uses HCC_TARGET_NAMESPACE when building the issuance URL', async () => {
    vi.resetModules()
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await issueMcpHostRuntimeTokens('chatllm')

    const [url] = (globalThis.fetch as unknown as ReturnType<typeof vi.fn>).mock.calls[0]
    expect(url).toBe(
      'http://control-api.test:8090/api/v1/auth/mcp-host/first-party-hosts/standalone/tokens'
    )
  })
  ...
  it('surfaces non-2xx responses with status code in the error message', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(/HTTP 401/)
  })
  ...
  it('surfaces non-2xx responses without a response body', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(
      'mcpHost runtime token issuance failed: HTTP 403 for host "chatllm"'
    )
  })
  ...
  it('surfaces non-2xx responses when reading the response body fails', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(
      'mcpHost runtime token issuance failed: HTTP 500 for host "chatllm"'
    )
  })
  ...
  it('surfaces request aborts as issuance timeouts', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(/timed out/)
  })
  ...
  it('aborts hung issuance requests after the request timeout', async () => {
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    const result = issueMcpHostRuntimeTokens('chatllm')
  ...
  it('rejects empty access/refresh in response body', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
    ...
    const issueMcpHostRuntimeTokens = await loadIssuer()
    await expect(issueMcpHostRuntimeTokens('chatllm')).rejects.toThrow(/missing required fields/)
  })
})
