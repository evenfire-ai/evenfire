...
import { decideMountStatus, type GfsMountIntent, resolveMount } from "./contextGfsMount";
...
describe("decideMountStatus (intent-not-authority)", () => {
  it("mounts when every requested scope is held", () => {
    ...
    expect(decideMountStatus(intent(["gfs.read", "gfs.write"]), new Set(["gfs.read", "gfs.write"]))).toBe(
      "Mounted"
    );
  });
  ...
  it("is PendingMount when a requested scope is NOT held (no escalation)", () => {
    ...
    expect(decideMountStatus(intent(["gfs.read", "gfs.write"]), new Set(["gfs.read"]))).toBe("PendingMount");
