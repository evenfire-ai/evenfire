...
export class NetworkPolicyReconciler {
  ...
  async reconcileExternalEgress(
    ...
    for (const binding of bindings) {
      ...
      // #299 Phase 2: the class is per-binding here (exact-host | provider), not
      // the helper's hardcoded 'exact-host'. Overridden at this call site so the
      // helper stays shared with the three sites that only render exact-host.
      policy.metadata!.labels![EGRESS_CLASS_LABEL] = egressClass
      if (stateAnnotations) {
        policy.metadata = {
          ...(policy.metadata ?? {}),
