...
| `CONTEXT_MAPPER_NETPOL_RESYNC_SEC`                | `0`                                             | Periodic NetworkPolicy coordinated convergence (seconds). Re-enters the single-flight pass; `0` disables the interval (merge default). Startup still converges once. **Enable is a separate ops decision** (issue #478): before any non-zero value, add a freshness gate **or** use a controller-runtime 10h-scale period. Do not treat `300` as the enable value while passes last minutes. |
| `CONTEXT_MAPPER_NETPOL_ORPHAN_DELETE_CAP`         | `10`                                            | Absolute orphan-delete cap for a namespace-wide NetworkPolicy sweep. Candidates **strictly above** this count refuse the pass (no deletes, no certification).                                                                                                                                                                                                                                |
| `CONTEXT_MAPPER_NETPOL_ORPHAN_DELETE_CAP_PERCENT` | `20`                                            | Percent orphan-delete cap against the listed managed fleet. Strict `>`. Inert on a tiny inventory where `percent * listed < 1`.                                                                                                                                                                                                                                                              |
