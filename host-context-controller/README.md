...
| `CONTEXT_MAPPER_K8S_API_CIDRS`                    | _(unset → `[]`)_                                | K8s API-server CIDRs for `allow-k8s-api-egress-*`. Empty falls back to `KUBERNETES_SERVICE_HOST`. **Validated fail-closed at module load**: a malformed entry, or one broader than `/24` (IPv4) or `/120` (IPv6), **crashes startup** rather than programming a permissive policy. IPv6 entries feed only the NetworkPolicies; the openai-compatible broker deny-set uses the IPv4 entries (the LAN classifier is IPv4-only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
