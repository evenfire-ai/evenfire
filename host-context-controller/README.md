...
| `CONTEXT_MAPPER_NETPOL_RESYNC_SEC`                | `0`                                             | Periodic NetworkPolicy coordinated convergence (seconds). Re-enters the single-flight pass; `0` disables the interval (merge default). Startup still converges once. Default `0` logs a `console.warn` on every boot (same pattern as Host/SFS/GFS/LlmHook) — expected; it names the self-heal gap. **Enable is a separate ops decision** (issue #478): before any non-zero value, add a freshness gate **or** use a controller-runtime 10h-scale period. Do not treat `300` as the enable value while passes last minutes.                                              |
| `CONTEXT_MAPPER_NETPOL_ORPHAN_DELETE_CAP`         | `10`                                            | Absolute orphan-delete cap for a namespace-wide NetworkPolicy sweep. Candidates **strictly above** this count refuse the pass (no deletes, no certification). Governs **every** `fullReconcile` (startup, event-driven, and the resync timer), not only a periodic tick. A legitimate bulk offboarding above the cap withholds certification until an operator raises it. Read-only census: `scripts/ops/hcc-netpol-orphan-census.sh` — `live_cap_would_trip` is Deployment env only; `controller_cap_would_trip` applies these compiled defaults when the env is unset. |
| `CONTEXT_MAPPER_NETPOL_ORPHAN_DELETE_CAP_PERCENT` | `20`                                            | Percent orphan-delete cap against the listed managed fleet. Strict `>`. Inert on a tiny inventory where `percent * listed < 1`. Same every-`fullReconcile` scope as the absolute cap.                                                                                                                                                                                                                                                                                                                                                                                    |
