...
describeRealPostgres('distinct catalog key pagination on real PostgreSQL', () => {
  ...
  it('preserves distinct keys for every duplicate-capable producer arm class', async () => {
    ...
    for (const mode of ['direct', 'team'] as const) {
      const actor = await principal(`sandbox-${mode}`, mode === 'team' ? 3 : 0)
      const recipes = ['a', 'b', 'c'].map(
        name => `${config.sandboxNamespace}/sandbox-${mode}-${name}`
      )
      const apps = ['a', 'b', 'c'].map(
        name => `${config.sandboxNamespace}/sandbox-app-${mode}-${name}`
      )
      for (const recipe of recipes)
        await operationalResource('workflow_recipe', recipe, 'workflow_recipe')
      for (const app of apps) await operationalResource('sandbox_app', app, 'workflow_recipe')
      for (let copy = 0; copy < 3; copy += 1) {
        await relationship({
          ...
          targetId: apps[0],
          instanceId: `sandbox-${mode}-a-${copy}`,
        })
      }
      for (const [index, app] of apps.slice(1).entries()) {
        await relationship({
          sourceType: 'workflow_recipe',
          sourceId: recipes[index + 1],
          relationshipType: 'exposes_sandbox_app',
          targetType: 'sandbox_app',
          targetId: app,
          instanceId: `sandbox-${mode}-${index + 1}`,
        })
      }
      if (mode === 'direct') {
        for (const recipe of recipes) {
          const [recipeNamespace, recipeName] = recipe.split('/')
          await databasePool.query(
            `INSERT INTO user_workflow_triggers(user_id, recipe_namespace, recipe_name) VALUES ($1, $2, $3)`,
            [actor.userId, recipeNamespace, recipeName]
          )
        }
      } else {
        for (const teamId of actor.teamIds) {
          const [recipeNamespace, recipeName] = recipes[0].split('/')
          await databasePool.query(
            `INSERT INTO team_workflow_triggers(team_id, recipe_namespace, recipe_name) VALUES ($1, $2, $3)`,
            [teamId, recipeNamespace, recipeName]
          )
        }
        for (const recipe of recipes.slice(1)) {
          ...
          await databasePool.query(
            `INSERT INTO team_workflow_triggers(team_id, recipe_namespace, recipe_name) VALUES ($1, $2, $3)`,
            [actor.teamIds[0], recipeNamespace, recipeName]
          )
        }
      }
      await consumeFamily({
