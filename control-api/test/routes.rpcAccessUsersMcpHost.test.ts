...
//
// NOTE: because requireValidRpcAccessTokenAny is stubbed to a pass-through
// here, the route's scope ALLOW-LIST is NOT exercised by this file. The
// genuine scope-gate assertions (including that a `host:model:write`-only
...
function buildApp(hosts: HostCRD[], rpcAuth?: { teamId?: string }, bindingBudgetMs?: number) {
  ...
  const bindingService = { bind: vi.fn() }
  const app = express()
  if (rpcAuth) {
  ...
  app.use(
    createRpcAccessUsersRouter(gatewayStub as unknown as K8sGateway, {
      bindingService,
      bindingBudgetMs,
    })
  )
...
describe('POST /rpc/access/users/:userId/mcp-hosts/:hostRef — atomic access and binding', () => {
  beforeEach(() => {
    svc.getUserAgents.mockReset()
    svc.getCurrentTeam.mockReset()
    svc.getCurrentTeam.mockImplementation(async (userId: string, teamId: string) => ({
      id: teamId,
      name: 'Team',
      role: 'member',
      userId,
  ...
  it('authorizes once, derives identity from verified claims, and records the binding', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    const teamId = '00000000-0000-4000-8000-000000000002'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp(
      [{ metadata: { name: 'host-a' }, spec: { enabled: true } }],
      { teamId }
    )
    bindingService.bind.mockResolvedValue({
      runId: DIRECT_BINDING_BODY.runId,
      status: 'created',
    ...
    expect(bindingService.bind).toHaveBeenCalledWith({
      ...DIRECT_BINDING_BODY,
      hostRef: 'host-a',
      identityIssuer: expect.any(String),
      actorHumanSub: userId,
      userId,
      teamId,
    })
  })

  it('rejects body-supplied identity and never records a binding', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp([
      { metadata: { name: 'host-a' }, spec: { enabled: true } },
    ...
    await request(app)
      .post(`/rpc/access/users/${userId}/mcp-hosts/host-a`)
      .send({ ...DIRECT_BINDING_BODY, userId })
      .expect(400)

    expect(bindingService.bind).not.toHaveBeenCalled()
  })

  it('fails closed before binding when the directory grant is absent', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: [] })
    const { app, bindingService, gatewayStub } = buildApp([
      { metadata: { name: 'host-a' }, spec: { enabled: true } },
    ])
  ...
  it('uses the route-local 2 KiB body ceiling', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp([
      { metadata: { name: 'host-a' }, spec: { enabled: true } },
    ...
    await request(app)
      .post(`/rpc/access/users/${userId}/mcp-hosts/host-a`)
      .send({ ...DIRECT_BINDING_BODY, sessionId: 'x'.repeat(3_000) })
      .expect(413)

    expect(bindingService.bind).not.toHaveBeenCalled()
  })

  it('returns an available Host connection when binding storage is transiently unavailable', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp([
      { metadata: { name: 'host-a' }, spec: { enabled: true } },
    ])
    bindingService.bind.mockRejectedValue(new Error('storage unavailable'))
  ...
  it('returns 409 when immutable attribution already exists for the run ID', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp([
      { metadata: { name: 'host-a' }, spec: { enabled: true } },
    ])
    bindingService.bind.mockRejectedValue(new DirectRunBindingConflictError())
  ...
  it('does not wait for the tracing statement timeout when a binding stalls', async () => {
    const userId = '00000000-0000-4000-8000-000000000001'
    svc.getUserAgents.mockResolvedValue({ userId, agentNames: ['host-a'] })
    const { app, bindingService } = buildApp(
      [{ metadata: { name: 'host-a' }, spec: { enabled: true } }],
    ...
    bindingService.bind.mockImplementation(() => new Promise(() => undefined))
    ...
    await request(app)
      ...
      .expect(200)
      .expect(({ body }) => {
        expect(body.bindingStatus).toBe('unavailable')
      })
  })
})
...
describe('GET /rpc/access/users/:userId/mcp-hosts/:hostRef — happy path', () => {
  ...
  it('returns 200 when host has spec.enabled=true explicitly', async () => {
    ...
    const { app } = buildApp([{ metadata: { name: 'chatllm' }, spec: { enabled: true } }])
...
describe('GET /rpc/access/users/:userId/mcp-hosts/:hostRef — team-level access (issue #141)', () => {
  ...
  it('returns 200 when user has no user_agents row but team has the agent (the core bug)', async () => {
    ...
    const { app } = buildApp([{ metadata: { name: 'development' }, spec: {} }], { teamId })
