 ...
 *     explicitly declares the `registryPullSecret` capability, so a user-facing route cannot
...
function createCoreApiMock() {
  return {
    ...
    createNamespacedSecret: vi.fn().mockResolvedValue({
      metadata: { name: PULL_SECRET, namespace: NS, uid: 'pull-uid', resourceVersion: '2' },
    }),
...
describe('platform pull secret vs. Secret write constraints', () => {
  ...
  it('WITH the platform opt-in it is accepted', () => {
    expect(() =>
      assertValidSecretConstraints(pullSecretRequest(), { capability: 'registryPullSecret' })
    ).not.toThrow()
  })

  it('createSecret accepts the real payload when the provisioner opts in', async () => {
    await expect(
      service.createSecret(pullSecretRequest(), { capability: 'registryPullSecret' })
  ...
  it('updateSecret accepts the real payload when the provisioner opts in', async () => {
    ...
    await expect(
      service.updateSecret(pullSecretRequest(), undefined, { capability: 'registryPullSecret' })
    ).resolves.toBeDefined()
