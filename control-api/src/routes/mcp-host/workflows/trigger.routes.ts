...
export function createMcpHostWorkflowTriggerRoutes(gateway: K8sGateway): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!caller) return
      if (!requireMcpHostControlScope(caller, res, 'workflow:trigger')) return
      ...
      try {
        ...
        if (result.kind === 'approval') {
          // Defensive for future caller modes. MCP-host control triggers
          // currently use approval_request_id to create approved runs directly.
