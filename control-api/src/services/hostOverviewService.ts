...
export class HostOverviewService {
  ...
  async getHostOverview(hostName: string, namespace?: string): Promise<HostOverview> {
    ...
    // A communication-channel's spec.hostRef is ALWAYS the host's RFC1123
    // metadata.name (see the channel wizard / CRD schema), never the display
    // name. Since PR #304 turned host.spec.host into an editable display label,
    // it must NOT be used to match channels: a display that collides with
    // another agent's metadata.name would surface that agent's channels — and
    // their recipients (telegram userIds / emails / slack userNames aggregated
    // below) — under the wrong agent. Match on the RFC1123 identifier only.
    const communicationChannels = allChannels.filter(ch => (ch.spec?.hostRef || '') === hostName)
