...
describe('internalControlSigner (WRC)', () => {
  beforeEach(() => {
  ...
  it('signs HS256 JWTs with WRC provisioner claims and a 60s TTL', () => {
    ...
    expect(parts[2]).toBe(expectedSignature(token, 'test-wrc-internal-control-secret'))
  ...
  it('rejects signing when the env secret is missing', () => {
    delete process.env.INTERNAL_CONTROL_JWT_WRC_HMAC_SECRET

    expect(() => signInternalControlJwt()).toThrow(/INTERNAL_CONTROL_JWT_WRC_HMAC_SECRET/)
  })
})
