 ...
 * receives it, parses that wire form with parseCodexCompletionRequestV1, and
 * hashes the validated projection — the same steps control-api authorize and
 * codex-llm-proxy perform. Callers send `value.request` with
 ...
 * hashCodexCompletionRequestV1(request). Never throws; invalid input fails closed.
 */
function hashCanonicalCodexRequest(raw) {
  ...
  return ok(Object.freeze({ request: parsed.value, requestHash: hashCodexCompletionRequestV1(parsed.value) }))
}
