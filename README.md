...
> **A self-hosted platform for LLM agents that do real work.** Connect tools,
> compose workflows, manage teams, and govern cost, all declared as Kubernetes
> resources. Bring your own model keys. Every risky action waits for a human
> "yes."
...
evenfire runs LLM agents that take real actions, and it runs them on your own
infrastructure. Agents chat across Telegram, Email, Slack, and a desktop app.
They call tools, both built-in (shell commands, a real browser, file read and
write, HTTP) and any connector you add over the
[Model Context Protocol](https://modelcontextprotocol.io) (MCP). They compose
multi-step workflows and draw on teams, shared files, and cost budgets. Before a
risky action runs, it pauses for a human to approve it.

Every part of the fleet is a Kubernetes custom resource: agents, connectors,
channels, workflows, and policies. That makes your whole platform
version-controlled, reviewable configuration. You bring your own model keys, and
your prompts and data stay in your environment.

One command (`make minikube-setup`) brings the whole platform up locally.

> **Naming:** the code uses the project's internal name **clerum**: `clerum.io`
> CRDs, `CLERUM_*` env vars, `clerum-*` packages. **evenfire** is the public name
> of the same project. Details:
...
evenfire is nine capabilities, each backed by code in this repo and declared as
configuration. Start anywhere; each one links to its depth.

- **Agents:** multi-channel agents (Telegram / Email / Slack / desktop) that take
  real actions: `shell_exec`, `http_request`, a real browser, an X11 desktop, and
  workspace files, plus artifact generation, persistent memory, and
  human-in-the-loop approvals on the risky calls. →
  [mcp-host/README.md](mcp-host/README.md)
- **Console & client:** the two surfaces you work in day to day. The **Control
  UI** console is where admins govern the fleet: token usage by team, model,
  agent, and desktop user; budgets and model prices; per-tool approval overrides;
  connector egress policy; a trust-rated connector registry. The Electron
  **Desktop App** is where people use the agents: chat, a live tool-call view,
  in-chat approvals, and artifacts, with no Telegram bot or curl required. (A
  third, smaller **Profile UI** is where invited members accept an invite and set
...
- **Connectors (MCP):** governed MCP servers with per-`Context` allowlists
  enforced by NetworkPolicy (not convention); any stdio tool plugs in through the
  bridge; remote SaaS connectors sit behind a pinned egress proxy. →
  [mcp-servers/README.md](mcp-servers/README.md),
  [docs/how-to/add-mcp-server.md](docs/how-to/add-mcp-server.md)
- **Workflows:** declarative, multi-workload `WorkflowRecipe`s with a lifecycle
  state machine: risk-based approval, shadow testing, automatic rollback, and
  multi-step coordinator pods. →
  [docs/crds/workflowrecipe.md](docs/crds/workflowrecipe.md)
- **Plugins & registry:** author and publish connectors and recipes to a governed
  registry (entries carry trust levels and an `author`); build custom coordinator
  images with `@clerum/workflow-sdk`; install through a governed flow. →
...
- **vs a hosted assistant:** you own the data and the infrastructure. Prompts,
  files, and model keys never leave your environment, and no vendor sits in the
  request path.
- **vs an in-process agent framework:** real isolation and governance. Per-agent
  pods, default-deny networking, multi-tenant teams and roles, and approvals
  enforced by the platform rather than by the calling application.
...
(shell, file read/write, HTTP, memory) need no MCP server, so install connectors
from the registry only when you want more.
...
message the `chatllm` agent, and ask it to run a command or generate a PDF, then
approve the tool call from the chat. The same password logs into the Control UI
as `admin`. The Desktop App is the client you just used, and the Control UI is
the admin console for the same fleet. Both are toured in
...
flowchart LR
    ...
    RP --> MH
    MH <--> LLM[("LLM provider<br/>OpenAI · Claude · Gemini · …")]
...
`host-context-controller` reconciles `Host`, `Context`, and `McpServer` CRDs into
Deployments, Services, and NetworkPolicies; the `workflow-recipes` operator does
the same for multi-step workflow workloads. Admins drive the same CRDs from the
Control UI console through `control-api`, so the UI writes the declarative
substrate directly instead of bypassing it. See
...
A model can be steered, misled, or simply wrong, so the platform never trusts it
by default. Four enforcement layers, all in this repo:
...
3. **Default-deny networking.** Runtime namespaces start deny-all in both
   ...
   are pinned behind a hardened egress proxy. The `http_request` tool is governed
   separately by the `CLERUM_HTTP_ALLOWLIST` domain allowlist (empty by default;
   set it, or every public domain is reachable). That allowlist, not IP
   filtering, is its egress control.
...
The whole fleet is eight `clerum.io` custom resources. You review, version, and
apply them with `kubectl` or GitOps, the same as any other Kubernetes config:
...
evenfire supports 30+ LLM providers behind one interface, including OpenAI,
Anthropic, Google (Gemini and Vertex AI), Amazon Bedrock, Azure OpenAI, Mistral,
Groq, xAI, DeepSeek, and more. A `Host` binds to one provider, and operators
allowlist the specific models it may use.

Keys live in a Kubernetes Secret you create (dev mode: environment variables).
For the full provider list, credentials, and setup, see the
...
unit-test file and case totals, so the count stays real instead of a hardcoded
number that drifts. E2E suites run against minikube with Calico NetworkPolicies
and the approval flow: [docs/testing/e2e-guide.md](docs/testing/e2e-guide.md).
