...
describe("routes/directory", () => {
  ...
  it("rejects bad bearer token", async () => {
    ...
    await request(app).get("/directory/search?q=alice").set("authorization", "Bearer bad-token").expect(401);
    expect(directoryServiceMock.searchDirectory).not.toHaveBeenCalled();
  });
  ...
  it("returns empty items when query is blank", async () => {
    ...
    const response = await request(app).get("/directory/search?q=").set("authorization", "Bearer good-token").expect(200);
  ...
  it("searches using authenticated teamId", async () => {
    ...
    expect(directoryServiceMock.searchDirectory).toHaveBeenCalledWith("team-1", "alice", "good-token");
  });
});
