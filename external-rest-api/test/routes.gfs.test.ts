...
describe('routes/gfs /me/gfs/* (user session passthrough → /external/gfs/*)', () => {
  ...
  it('preserves the allowlisted no-escalation reason in a public 403 envelope', async () => {
  ...
  it('maps control-api gfsc 5xx failures to bounded public envelopes', async () => {
    ...
    for (const [status, error, code, message] of [
      [504, 'gfsc_timeout', 'upstream_timeout', 'The upstream service timed out.'],
      [
        502,
        'gfsc_unreachable',
...
describe('GET /me/gfs/grants (delegation list passthrough)', () => {
  ...
  it('preserves the allowlisted manage-acl reason in a public 403 envelope', async () => {
    clientMock.controlApiRequest.mockRejectedValue(
