...
export async function findMembership(userId: string, teamId: string): Promise<MembershipRow | null> {
  const result = await pool.query(
...
export async function upsertProfile(userId: string, displayName: string | null, channelsJson: string): Promise<ProfileRow> {
  const result = await pool.query(
