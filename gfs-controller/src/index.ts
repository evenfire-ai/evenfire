...
import { createGfsAuthorityCheckpointer } from './auth/actionAuthority'
...
async function assertUploadV2Ready(
  ...
  if (storageRole === 'writer') {
    ...
    if (!info.isDirectory())
      throw new Error('[gfsc] GFS_UPLOAD_V2_ENABLED requires a writable GFS storage directory')
    ...
    for (const ancestor of [resolve(storageMountPath), uploadRoot]) {
      ...
      if (!ancestorInfo.isDirectory())
        throw new Error('[gfsc] GFS upload storage ancestors must be real directories')
    }
...
function startInvalidation(
  ...
  const connect = async (): Promise<void> => {
    ...
    const listener = new InvalidationListener({
      client: asListenClient(client),
      cache,
      onDegraded: reconnect,
    })
    try {
      ...
      console.error(
        `[gfsc] invalidation LISTEN failed (cache stays bypassed), retrying: ${errMsg(err)}`
      )
      reconnect()
    }
  }
  ...
  if (config.publicKey.trim() !== '') {
    ...
    const writeService =
      config.storageRole === 'writer'
        ...
        : undefined
    const checkpointAuthority = createGfsAuthorityCheckpointer({
      baseUrl: config.controlApiBaseUrl,
      serviceToken: config.controlApiServiceToken,
      timeoutMs: config.authorityCheckpointTimeoutMs,
    })
    ...
    serving = new GfsServingHandler({
      ...
      ...(writeService
        ? {
            copy: {
              authorizeMany: (ctx, requests, budget) =>
                permissions.authorizeMany(ctx, requests, budget),
        ...
        ? {
            uploadService: (uploadServiceForCleanup = new GfsUploadSessionService({
              ...
              authorize: async (principal, operation, targetRid) => {
                ...
                if (!decision.allowed)
                  throw new GfsError('forbidden', `not authorized to ${operation} upload`)
              },
