...
function fixture(
  options: {
    ...
    omitLastParent?: boolean
    denyChild?: boolean
    copyError?: unknown
    scopes?: GfsVerifiedClaims['scopes']
    times?: number[]
  } = {}
) {
  const snapshot = [node()]
  if (options.child)
    snapshot.push(
      node({
        ...
        name: 'child',
        pathCache: '/source/child',
        depth: 1,
      })
    )
  for (
    let index = snapshot.length;
    index < (options.snapshotCount ?? snapshot.length);
    index += 1
  ) {
    ...
    snapshot.push(
      node({
        ...
        parentResourceId: SOURCE,
        name: `child-${index}`,
        pathCache: `/source/child-${index}`,
        depth: 1,
      })
    )
  }
  if (options.omitLastParent && snapshot.length > 1)
    snapshot[snapshot.length - 1].parentResourceId = ROOT
  const destination: CopyDestinationSnapshot = {
    ancestors: [
      node({
        resourceId: DEST,
        parentResourceId: ROOT,
    ...
    copy: {
      authorizeMany: async (_ctx, requests) => {
        ...
        return requests.map(item => ({
          allowed: !(options.denyChild && item.resourceId === CHILD.replaceAll('-', '')),
        }))
      },
      permissionEpoch: () => ({ generation: 0, bypassed: false }),
      resources: {
        loadCopyAdmission: async () => ({
          sourceRoot: snapshot[0] ?? null,
          destinationParent: destination.ancestors[0] ?? null,
        }),
        loadCopySnapshot: async () => {
          loads += 1
          return snapshot
        },
        loadCopyDestination: async () => {
          loads += 1
          return destination
        },
      },
...
describe('POST /v1/copy', () => {
  it('rejects v2 source or destination substitution before copy publication', async () => {
    ...
    f.deps.verifyToken = () => ({
      ...
      actionAuthority: {
        binding: {
          ...
          resource: { type: 'gfs_resource', logicalId: SOURCE },
          target: {
            drive: 'main',
            action: 'copy',
            resourceId: SOURCE,
          ...
          behaviorBindingHash: 'abh1_copy_behavior',
        },
        sourceIssuedAt: now,
    ...
    f.deps.checkpointAuthority = async () => undefined
    const res = new Response()

    await f.handler.tryHandle(
      request({
        drive: 'main',
        sourceResourceId: CHILD,
  ...
  it('does not extend a long deadline when its first timer callback is delayed', async () => {
    ...
    const monotonicClock = vi
      ...
      .mockImplementation(() => monotonicNowNs)
    try {
      const f = fixture()
  ...
  it('clears a long copy timer when the request completes', async () => {
    ...
    try {
      ...
      await f.handler.tryHandle(
        request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
        res as never
      )
  ...
  it('adopts one request id, batch-authorizes the frozen tree and returns a minimal DTO', async () => {
    ...
    expect(f.copy.mock.calls[0][0]).toMatchObject({
      requestId: REQUEST,
      subject: 'agent:test',
      destinationParentId: DEST.replaceAll('-', ''),
    })
  ...
  it('returns one generic denial when a descendant is denied', async () => {
    const f = fixture({ child: true, denyChild: true })
    const res = new Response()
    await f.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      res as never
  ...
  it('is absent on readers and maps ENOSPC without leaking storage details', async () => {
    ...
    delete reader.deps.writeService
    delete reader.deps.copy
    const absent = new Response()
    await reader.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      absent as never
    )
    ...
    const writer = fixture({
      copyError: Object.assign(new Error('private filesystem path'), { code: 'ENOSPC' }),
    })
    const full = new Response()
    await writer.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      full as never
    )
  ...
  it('rejects a body-drive mismatch before loading either tree', async () => {
    ...
    const res = new Response()
    await f.handler.tryHandle(
      request({ drive: 'other', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      res as never
    )
  ...
  it('uses the admission timestamp for timeout and audits only after batch authorization', async () => {
    const timed = fixture({ times: [0, 0, 0, 30_000] })
    const timedRes = new Response()
    await timed.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      timedRes as never
    )
    ...
    const ceiling = fixture({ scopes: ['gfs.read'] })
    const denied = new Response()
    await ceiling.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      denied as never
    )
  ...
  it('enforces If-Match, collision and object limits after authorization', async () => {
    const stale = fixture()
    const staleRes = new Response()
    const staleAudit = vi.spyOn(stale.deps.copy!.audit, 'record')
    await stale.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST }),
      staleRes as never
    )
    ...
    await collision.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      collisionRes as never
    )
    expect(collisionRes.statusCode).toBe(409)

    const limited = fixture({ child: true })
    limited.deps.copy!.maxObjects = 1
    const limitedRes = new Response()
    await limited.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      limitedRes as never
    )
    ...
    const exact = fixture()
    exact.deps.copy!.maxObjects = 1
    const exactRes = new Response()
    await exact.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      exactRes as never
    )
  ...
  it('rejects the bounded one-over snapshot without publishing', async () => {
    const fixture1001 = fixture({ snapshotCount: 1001 })
    const res = new Response()
    await fixture1001.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      res as never
    )
  ...
  it('rejects an unordered one-over sentinel before validating its omitted parent or publishing', async () => {
    const fixture1001 = fixture({ snapshotCount: 1001, omitLastParent: true })
    const res = new Response()
    await fixture1001.handler.tryHandle(
      request({ drive: 'main', sourceResourceId: SOURCE, destinationParentId: DEST, ifMatch: 2 }),
      res as never
