...
6. **Recipe McpServer CRDs sanitized by HCC** before any Deployment is created
...
sequenceDiagram
    ...
    WRC->>K8s: Create McpServer CRDs (→ mcp-server)
    WRC->>K8s: Create WRC-owned runtime NetworkPolicies when workflow pods need them

    K8s->>HCC_S: Watch: new McpServer CRD
    Note over HCC_S: Sanitization gate

    alt PASS
        HCC_S->>K8s: Create Deployment + Service
    else REJECT
        HCC_S->>K8s: Status → Rejected
    end
...
sequenceDiagram
    ...
    Note over HCC: SANITIZATION GATE
    HCC->>HCC: Check image registry
    HCC->>HCC: Check resource limits
    HCC->>HCC: Check security context
    HCC->>HCC: Check host access

    alt ALL CHECKS PASS
        HCC->>K8s: Create Deployment + Service
        HCC->>K8s: status.phase = Running
        HCC->>K8s: status.sanitization.passed = true
    else ANY CHECK FAILS
        HCC->>K8s: status.phase = Rejected
        HCC->>K8s: status.sanitization.reason = "..."
        Note over HCC: Operator notified via status
    end
```
...
flowchart TD
    ...
    API -->|"Creates WorkflowRecipe CRD"| WRC["WRC (within HCC)<br/>(validates, resolves inputs, topo sort)"]
    ...
    HCCS --> GATE{"3. Sanitization Gate<br/>(registry, limits, security)"}
    GATE -->|"PASS"| DEPLOY["Deployment + Service<br/>created in mcp-server"]
    GATE -->|"REJECT"| REJ["status.phase: Rejected<br/>Operator notified"]
...
flowchart TD
    ...
    STATIC -->|"ships static infrastructure exceptions"| L1["L1 — infrastructure<br/>DNS egress, HCC API, K8s API"]
    ...
    L0 -->|"applied to"| RT["mcp-host, mcp-server,<br/>sandbox-recipes, rpc-proxy"]
    L1 -->|"applied to"| RT
...
**Applied to**: `mcp-host`, `mcp-server`, `sandbox-recipes`, `rpc-proxy` — all 4 runtime namespaces.
...
**Applied to**: All 4 runtime namespaces (same scope as L0).
...
The timeout is configurable via `NETWORK_READY_TIMEOUT_MS` (default: 30,000ms).
...
flowchart LR
    ...
    GW -->|"HTTP POST /message<br/>(direct, via NetworkPolicy)"| MH["mcp-host<br/>(Agent + LLM)"]
...
> The gateway already has its own auth model (bot tokens + allowed sender lists).
...
flowchart LR
    ...
    RPC -->|"MCP Server Proxy<br/>/rpc/mcp/{server}"| MCP["MCP Servers"]
    RPC -->|"MCP Host Proxy<br/>/rpc/agent"| MH["mcp-host<br/>(Agent channel)"]
...
1. **WRC source lives in `workflow-recipes/`** — the reconciler module is loaded into the HCC process at runtime, though the source code remains in its own directory for development clarity.
