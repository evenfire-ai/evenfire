...
| 2026-08-26 | 2 | Category C implemented. Member + team detail "Contexts" tabs → route `access` / label **"Access"**; rows now show resolved labels (owning agent name(s) → context `displayName` → muted raw id) and no longer link into `/contexts`; confirm/toast/error copy reworded ("Access updated.", "Removed access" for deleted mappings); add-modal relabeled with resolved-label picker, writing context IDs unchanged. Team wizard step "Contexts" → **"Access"** with resolved-label options; `createFlowLoading.createTeam` updated. Legacy `ProfileAdminPanel.tsx` deleted (zero importers). `ProfileAdminHome` "Contexts" column → **"Access"** (same sort/count source `teamContextCounts`); org-delete warning reworded. Redirects added for `/users-and-teams/{users,teams}/:id/contexts` → `…/access`. Full vitest + `tsc` green. No backend changes (GET/PUT `/admin/{users,teams}/:id/contexts` payloads byte-identical). |
