...
| D10 | Access tab naming (supersedes D8's naming) | Operator correction: the merged tab is **"Agents"** at the `agents` route — NOT "Access". Semantics unchanged from D8 (agent rows + Connectors column + composite write); copy is agent-flavored (`Add agents`, `Remove Agent` confirm, `Agents updated.` / `Team agents updated.` toasts, `Agent` column header, `No agents assigned yet.`). Team wizard step is `Agents`; teams table shows a single sortable `Agents` column (teamAgentCounts). Redirects: `/contexts` AND `/access` tab URLs → `…/agents` |
| D9 | Deleted-mapping history in UI | **Removed entirely.** The "Removed access" tombstone section (backend `deletedContextIds`/`deletedAgentNames` soft-delete history) is operator noise — internal reconciliation bookkeeping, not actionable state — so the Access tabs no longer render it. The backend keeps its guard data untouched; the UI just stops displaying archaeology. Verified by the rewritten `member-removed-access` journey (deleted scope ⇒ zero residue: no section, no row, no raw id) |
...
| 2026-08-28 | merge | Merged `origin/dev` (Codex grant work, control-ui defect sweep, Button/RowActions primitives, token baseline). Conflicts resolved per operator-approved plan: HostWizard keeps BOTH imports (our privateContext + dev's llmCredentialSelect); McpServerTable button = our renamed class + dev's `--secondary` removal; RegistryInstallModal stays DELETED (dev's change was a one-line token rename in a file with zero production importers); package manifests took dev's version numbers with ALL our qa:recorder scripts preserved (57 scripts). Verified post-merge: control-ui `tsc` clean, playwright typecheck clean, **unit suite 1872/1872** (dev brought 170 new tests), and 8 recorder journeys green live (connectors ×4, member/team agents, team-create, agent-connectors). |
| 2026-08-28 | D10-fix | Operator caught a REAL bug we shipped: the member view still rendered a dead "Access" tab beside "Agents" — the D10 rename had silently no-opped on the member page's tab block (formatting drift, unasserted replace) and no spec guarded member-view tab absence. Fixed (UserTab now contact/approval-dms/communication-channels/agents/teams, zero 'access' remnants) + added a regression guard to the member journey asserting NO 'Access' tab renders. All 3 member journeys re-verified green against the live stack. Lesson recorded: every scripted replace must assert, and every removal needs an absence guard. |
| 2026-08-28 | D10 | Operator renamed the merged tab back to **Agents** (D8 semantics, new name): route `agents`, agent-flavored copy, wizard `Team → Members → Agents`, single `Agents` column on the teams table, redirects `/contexts` + `/access` → `/agents`. One post-merge formatting miss (users `[tab]` route list) was caught live by the journeys and fixed. Verified: `tsc` clean + all 7 affected recorder journeys green against the mock stack (full unit rerun was interrupted by an operator abort; suite was green at D9 and no unit-covered code changed since). |
| 2026-08-28 | D9 | Operator review killed the "Removed access" tombstone section on member/team Access tabs (backend soft-delete history = bookkeeping, not UI). Section + dead CSS removed from both pages; `member-removed-access` journey rewritten to assert ZERO residue after an out-of-band scope deletion (no heading, no row, no raw id, clean empty state); `team-access` comment updated. Verified: `tsc` clean, unit suite green, 4 affected recorder journeys green against the mock stack (member-access ×3 incl. removed-access, team-access). |
