...
| Understand the Control UI surface          | [LLM management](../control-ui/llm-management-source-of-truth.md) |
...
2. **mcp-host never calls control-api for this.** The NetworkPolicy denies that
   path by design. The allowlist reaches the runtime as a ConfigMap, not an API
   call, so an control-api outage cannot break a running agent.
...
slots are all present; `CLERUM_MODEL_NAME` overrides the model.
...
configured or not — is passed to a tool subprocess. The exclusion set is derived
from the full slot list of every provider _plus_ whatever the LLM Secret actually
delivers, so dynamically-named fallback slots are covered too.
