...
type ProviderAuthResult =
  | { ok: true; slackTarget?: SlackTargetContext }
...
function rateLimitAllowed(
  ...
  const now = Date.now()
  const key = medium === 'slack' && targetId ? `${medium}:${targetId}` : `${medium}:${peerKey(req)}`
...
function slackCanUseEvent(target: SlackTargetContext, payload: unknown, channelId?: string | null): boolean {
...
function slackCanUseEnrollmentEvent(
  ...
  const eventType = slackEventType(payload)
  return eventType === 'app_mention' || (eventType === 'message' && slackEventStartsWithMention(payload))
}
...
function slackDecisionActionLabel(command: NonNullable<ReturnType<typeof normalizeProviderDecision>>): string {
...
async function providerAuthorized(
  ...
  if (medium === 'telegram') {
    ...
    return ok
      ? { ok: true }
      : { ok: false, status: 401, error: 'invalid_provider_signature' }
  }
  if (medium === 'slack') {
