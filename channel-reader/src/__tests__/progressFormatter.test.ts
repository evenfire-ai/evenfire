...
import { formatProgressUpdate, formatFinalMessage } from "../progressFormatter";
...
describe("formatProgressUpdate", () => {
  ...
  it("shows iteration divider when iteration changes", () => {
    const steps = [
      step({ iteration: 0 }),
      step({ toolCallId: "c2", iteration: 1, stepIndex: 0 }),
    ];
...
describe("formatFinalMessage", () => {
  it("prefixes response with compact summary", () => {
    const steps = [step(), step({ toolCallId: "c2", toolName: "mongodb-server__aggregate", stepIndex: 1, totalSteps: 2 })];
