...
| `CLERUM_PROFILE_UI_URL`              | No        | -                                      | Base URL of the Profile UI. When set, the unresolved-sender notice ends with this link so an unverified Slack sender can find the account they need to link; when unset the notice still sends, just without a link.                                                       |
