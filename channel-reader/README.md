...
| `CLERUM_POLL_INTERVAL_SECONDS`       | No        | `2`        | Seconds between polling cycles. Telegram messages land in an in-memory queue via grammY long-polling and `pollCycle` only drains it. Slack and Email are fetched *during* the cycle (Slack `conversations.history`, IMAP), so this interval bounds their delivery latency. |
