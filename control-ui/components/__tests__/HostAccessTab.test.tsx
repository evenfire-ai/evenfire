...
describe('HostAccessTab — extracted access behavior', () => {
  ...
  it('renders Members by default and lists existing members', async () => {
    ...
    expect(screen.getByText('Grant or revoke which members can use this agent.')).toBeInTheDocument()
  ...
  it('switches to Teams sub-tab and shows the team description', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Grant or revoke team-level access to this agent.')).toBeInTheDocument()
    })
  ...
  it('grants member access end-to-end after Add member modal submission', async () => {
    ...
    fireEvent.click(
      dialog.querySelector('button.cu-btn--primary') as HTMLButtonElement
    )
