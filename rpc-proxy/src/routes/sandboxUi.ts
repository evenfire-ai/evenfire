...
function requireSandboxOAuthIdentity(req: AuthedRequest, res: Response, next: NextFunction): void {
  if (tokenDeclaresV2(extractAuthToken(req))) {
    requireRpcAuth(req, res, next)
    return
  }
  requireSandboxUiCookieSession(req, res, next)
}

function requireSandboxOAuthAuthority(req: AuthedRequest, res: Response, next: NextFunction): void {
...
const DISTRIBUTED_ADMISSION_RESPONSE_HEADERS = [
  'retry-after',
  'x-ratelimit-limit',
...
function forwardDistributedAdmissionHeaders(upstream: globalThis.Response, res: Response): void {
  if (upstream.status !== 429) return
  for (const header of DISTRIBUTED_ADMISSION_RESPONSE_HEADERS) {
...
export function createSandboxUiSessionRouter(): Router {
  ...
  router.post(
    ...
    async (req: AuthedRequest, res: Response) => {
      ...
      // surface.
      forwardDistributedAdmissionHeaders(upstream, res)
      res
  ...
  router.all(
    ...
    async (req: Request, res: Response) => {
      ...
      // ClientRequest. Each request gets persistent proxy listeners filtered
      // by request identity and detached when its response closes or finishes.
      const onProxyReq = (
