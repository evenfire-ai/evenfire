...
# FileReference v1 contract and byte classifier from the same package, so a
# change to it must rebuild both consumers.
assert_gfs_interaction_policy_change_rebuilds_consumers() {
  ...
  output="$(node -e '
    import("'"$REPO_ROOT"'/scripts/release/images-manifest.mjs").then(async m => {
      ...
      const wfPath = "'"$REPO_ROOT"'/.github/workflows/build-publish.yml"
      const wf = fs.readFileSync(wfPath, "utf8")
      const sourcePath = "packages/gfs-interaction-policy/**"
      const expectedFilters = ["control-ui", "mcp-host"]
      const expectedImages = [
        "control-ui",
        "mcp-host",
        "mcp-host-desktop",
  ...
  rc=$?
  if [ "$rc" -ne 0 ]; then
    fail "gfs-interaction-policy changed-path contract parse failed: $output"
  elif [ -z "$output" ]; then
    pass "a gfs-interaction-policy change rebuilds Control UI and the mcp-host variants"
  else
    fail "gfs-interaction-policy changed-path contract failed: $output"
  fi
}
