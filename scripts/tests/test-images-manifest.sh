...
assert_llm_provider_attempt_contract_change_rebuilds_consumers() {
  ...
  output="$(node -e '
    import("'"$REPO_ROOT"'/scripts/release/images-manifest.mjs").then(async m => {
      ...
      const expectedFilters = ["codex-llm-proxy", "control-api", "mcp-host"]
      const expectedImages = [
        "codex-llm-proxy",
        "control-api",
        "mcp-host",
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    pass "a provider-attempt-contract change rebuilds Control API, mcp-host variants, and the proxy"
  else
