...
sample_assertion="$(sed -n '/^assert_hcc_sampled_recoveries() {/,/^}/p' "$ROOT/scripts/e2e/e2e-hcc-watch-churn-readiness.sh")"
[[ -n "$sample_assertion" ]]
eval "$sample_assertion"
ok() { echo PASS >> "$tmp/sample-results"; }
fail() { echo FAIL >> "$tmp/sample-results"; }
churn_503=2 churn_transitions=1 MIN_CHURN_RECOVERIES=2
assert_hcc_sampled_recoveries
grep -q FAIL "$tmp/sample-results"
: > "$tmp/sample-results"
churn_transitions=2
assert_hcc_sampled_recoveries
[[ "$(grep -c PASS "$tmp/sample-results")" = 2 ]]
: > "$tmp/sample-results"
MIN_CHURN_RECOVERIES=0 churn_503=0 churn_transitions=0
assert_hcc_sampled_recoveries
