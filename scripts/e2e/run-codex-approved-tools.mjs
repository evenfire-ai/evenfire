...
function main() {
  ...
  const canonical = fs.realpathSync(required('APPROVED_TOOLS_CANONICAL_ROOT'))
  const runRoot = fs.realpathSync(path.join(canonical, '.local-notes/infra/runs'))
  if (path.relative(canonical, runRoot) !== '.local-notes/infra/runs')
    throw new Error('Canonical evidence root must not escape through a symlink')
  const evidence = fs.realpathSync(required('APPROVED_TOOLS_EVIDENCE_DIR'))
  const relative = path.relative(runRoot, evidence)
  if (!relative || relative.startsWith('..') || path.isAbsolute(relative))
    throw new Error('Evidence must be a dedicated canonical .local-notes/infra/runs child')
  const report = path.join(evidence, `approved-tools-${mode}.json`)
  const artifacts = path.join(evidence, `artifacts-${mode}`)
  if (
    fs.lstatSync(report, { throwIfNoEntry: false }) ||
    fs.lstatSync(artifacts, { throwIfNoEntry: false })
  )
    throw new Error('Refusing to reuse an existing report; choose a fresh evidence directory')
  process.umask(0o077)
  const env = {
    ...process.env,
  ...
  const verified = validateReport(JSON.parse(fs.readFileSync(report, 'utf8')))
