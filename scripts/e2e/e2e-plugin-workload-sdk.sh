...
load_branch_profile_ports() {
  ...
  if [[ "$ctx" =~ ^clerum-(codex|detached)- ]] || [[ "$ctx" =~ ^clerum-.+-[0-9a-f]{7,8}$ ]]; then
    if [ -z "$ports_env" ]; then
      die "Branch-owned context ${ctx} requires ports.env generated by scripts/minikube-profiles/branch.mk; set CLERUM_PROFILE_PORTS_ENV or E2E_PROFILE_PORTS_ENV before running this gate"
    fi
