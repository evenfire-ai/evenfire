...
# A branch-scoped context is only valid with the port-forward map generated by
# clerum/.local-notes/minikube-profiles/branch.mk for that exact context. A
# non-empty URL is not sufficient: another profile can be healthy on the same
# machine and would otherwise make a Desktop run appear to validate the wrong
# cluster.
require_branch_profile_urls() {
  local context="$1"
  local ports_env="${2:-${HOME}/.cache/clerum/minikube-profiles/${context}/ports.env}"
  if ! is_branch_scoped_e2e_context "$context"; then
    return 0
  fi
  if [[ ! -f "$ports_env" ]]; then
    echo "Refusing E2E for branch context ${context}: profile ports.env is missing at ${ports_env}" >&2
    return 1
  fi
  ...
  local pair name expected_key actual expected
  for pair in \
    'CONTROL_UI_BASE_URL CONTROL_UI_URL' \
    'CONTROL_API_BASE_URL CONTROL_API_URL' \
    'EXTERNAL_REST_API_BASE_URL EXTERNAL_REST_API_URL' \
    'RPC_PROXY_BASE_URL RPC_PROXY_URL'; do
    name="${pair%% *}"
    expected_key="${pair#* }"
    actual="${!name:-}"
    expected="$(profile_url "$expected_key")"
    if [[ -z "$expected" ]]; then
      echo "Refusing E2E for branch context ${context}: ${expected_key} is missing from ${ports_env}" >&2
      return 1
    fi
    if [[ "$(normalize_url "$actual")" != "$(normalize_url "$expected")" ]]; then
      echo "Refusing E2E for branch context ${context}: ${name}=${actual} does not match ${expected_key} from ${ports_env}" >&2
      return 1
    fi
  done
}
